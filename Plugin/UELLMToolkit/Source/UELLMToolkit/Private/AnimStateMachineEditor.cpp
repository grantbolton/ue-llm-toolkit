	...
	FString& OutError)
{
	...
	return CreateStateMachine(AnimBP, AnimGraph, StateMachineName, Position, OutNodeId, OutError);
}
	...
	FString& OutError)
{
	...
	if (!TargetGraph)
	{
		OutError = TEXT("Invalid target graph");
		return nullptr;
	}

	if (!TargetGraph->IsA<UAnimationGraph>())
	{
		OutError = FString::Printf(TEXT("Target graph '%s' is not an animation graph (class: %s). "
			"State machines can only be created in UAnimationGraph instances."),
			*TargetGraph->GetName(), *TargetGraph->GetClass()->GetName());
		return nullptr;
	}
