	...
	FString& OutError)
{
	...
	// Connect to output pose
	if (!FAnimGraphEditor::ConnectToOutputPose(StateGraph, NodeId, OutError))
	{
		return false;
	}

	// Bind BlendSpace parameters to variables
	for (const auto& Binding : ParameterBindings)
	{
		FString VarNodeId;
		if (!FAnimGraphEditor::BindVariable(
			AnimBP, Binding.Value, NodeId, Binding.Key,
			FString(), VarNodeId, OutError, StateMachineName, StateName))
		{
			return false;
		}
	}
	...
	FString& OutError)
{
	...
	// Bind the BlendSpace1D parameter to a variable
	if (!ParameterBinding.IsEmpty())
	{
		FString PinName = TEXT("X");

		FString VarNodeId;
		if (!FAnimGraphEditor::BindVariable(
			AnimBP, ParameterBinding, NodeId, PinName,
			FString(), VarNodeId, OutError, StateMachineName, StateName))
		{
			return false;
		}
