...
void FMCPTaskQueue::ExecuteTask(TSharedPtr<FMCPAsyncTask> Task)
{
	...
	TSharedPtr<IMCPTool> Tool = ToolRegistry->FindToolShared(Task->ToolName);
	if (!Tool.IsValid())
	{
	...
	else if (Task->TimeoutMs > 30000)
	{
		...
		FEvent* RawEvent = FPlatformProcess::GetSynchEventFromPool();
		if (!RawEvent)
		{
			Result = FMCPToolResult::Error(TEXT("Failed to allocate sync event for tool execution"));
		...
		TSharedPtr<FEvent, ESPMode::ThreadSafe> CompletionEvent = MakeShareable(RawEvent,
