...
public:
	...
	/** Find a tool by name (returns raw pointer, nullptr if not found) */
	...
	/** Find a tool by name (returns TSharedPtr, safe for lambda capture) */
	TSharedPtr<IMCPTool> FindToolShared(const FString& ToolName) const
	{
		const TSharedPtr<IMCPTool>* Found = Tools.Find(ToolName);
		return (Found && Found->IsValid()) ? *Found : nullptr;
	}
