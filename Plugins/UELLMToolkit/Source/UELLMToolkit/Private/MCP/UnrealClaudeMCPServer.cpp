...
bool FUnrealClaudeMCPServer::HandleExecuteTool(const FHttpServerRequest& Request, const FHttpResultCallback& OnComplete)
{
	...
	// Tool execution must happen on the game thread (UObject/Blueprint access).
	// The HTTP server dispatches handlers on a listener thread, so we need to
	// bounce to the game thread and block until the result is ready.
	FMCPToolResult Result;
	if (IsInGameThread())
	{
		Result = ToolRegistry->ExecuteTool(ToolName, ParamsJson.ToSharedRef());
	}
	else
	{
		TSharedPtr<FMCPToolRegistry> Registry = ToolRegistry;
		TSharedRef<FJsonObject> ParamsRef = ParamsJson.ToSharedRef();
		FEvent* CompletionEvent = FPlatformProcess::GetSynchEventFromPool(true);
		if (!CompletionEvent)
		{
			Result = FMCPToolResult::Error(TEXT("Failed to allocate sync event"));
		}
		else
		{
			AsyncTask(ENamedThreads::GameThread, [Registry, ToolName, ParamsRef, &Result, CompletionEvent]()
			{
				Result = Registry->ExecuteTool(ToolName, ParamsRef);
				CompletionEvent->Trigger();
			});

			const bool bSignaled = CompletionEvent->Wait(UnrealClaudeConstants::MCPServer::GameThreadTimeoutMs);
			FPlatformProcess::ReturnSynchEventToPool(CompletionEvent);

			if (!bSignaled)
			{
				Result = FMCPToolResult::Error(FString::Printf(
					TEXT("Tool '%s' execution timed out after %d seconds"),
					*ToolName, UnrealClaudeConstants::MCPServer::GameThreadTimeoutMs / 1000));
			}
		}
	}
...
bool FUnrealClaudeMCPServer::HandleStatus(const FHttpServerRequest& Request, const FHttpResultCallback& OnComplete)
{
	...
	ResponseJson->SetStringField(TEXT("projectDirectory"), FPaths::ConvertRelativePathToFull(FPaths::ProjectDir()));
